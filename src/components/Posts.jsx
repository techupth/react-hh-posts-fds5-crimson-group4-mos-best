...
function Posts() {
  ...
  const delLike = (postId) => {
    setLikes((prevLikes) => ({
      ...prevLikes,
      [postId]: Math.max((prevLikes[postId] || 0) - 1, 0),
    }));
  };
  return (
          ...
          <div className="post-item" key={item.id}>
            <div className="post-header">
              ...
              <div className="post-social-media-stats">
                ...
                <span className="post-likes">{likes[item.id]}</span>
              </div>
            </div>
            <p className="post-content">{item.content}</p>
            <div className="post-actions">
              <button className="like-button" onClick={() => addLike(item.id)}>
                Like
              </button>
              <button
                className="dislike-button"
                onClick={() => delLike(item.id)}
              >
                Dislike
              </button>
            </div>
